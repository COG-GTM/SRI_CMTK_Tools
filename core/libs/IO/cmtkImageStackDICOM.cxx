...
namespace cmtk
{
    ...
    ImageStackDICOM::AddImageFile(ImageFileDICOM::SmartConstPtr &newImage)
    {
        iterator it = begin();
        for (; it != end(); ++it)
            if (newImage->m_InstanceNumber < (*it)->m_InstanceNumber)
                break;
        insert(it, newImage);
    }
        ...
        static const wsLookupType wsLookup[] =
            {
                ...
                {"dicom:SoftwareVersions", {"\t", NULL, NULL, "\n"}},
                ...
                {"dicom:Modality", {"\t", NULL, NULL, "\n"}},
                {"dicom:FlipAngle", {"\t", NULL, NULL, "\n"}},
                {"dicom:MagneticFieldStrength", {"\t", NULL, NULL, "\n"}},
                {"dicom:PhotometricInterpretation", {"\t", NULL, NULL, "\n"}},
                {"dicom:AcquisitionMatrix", {"\t", NULL, NULL, "\n"}},
                {"dicom:SliceThickness", {"\t", NULL, NULL, "\n"}},
                {"dicom:Columns", {"\t", NULL, NULL, "\n"}},
                {"dicom:Rows", {"\t", NULL, NULL, "\n"}},
                {"dicom:PixelSpacing", {"\t", NULL, NULL, "\n"}},

                {"dwellTime", {"\t", NULL, NULL, "\n"}},
                {"phaseEncodeDirection", {"\t", NULL, NULL, "\n"}},
                {"phaseEncodeDirectionSign", {"\t", NULL, NULL, "\n"}},
                {"dicom:SequenceName", {"\t", NULL, NULL, "\n"}},
                {"GE:PulseSequenceName", {"\t", NULL, NULL, "\n"}},
                {"GE:PulseSequenceDate", {"\t", NULL, NULL, "\n"}},
                {"GE:InternalPulseSequenceName", {"\t", NULL, NULL, "\n"}},
                {"GE:EffectiveEchoSpacing", {"\t", NULL, NULL, "\n"}},
    ...
    const std::string
    mungeXMLNs(const std::string &badNs, const std::string &goodNs, const std::string &elementName, const ImageOperationXMLOptions &options)
    {
        std::string tagWithNs;
        if (options.m_strictXML)
        {
            tagWithNs = goodNs + ":" + elementName;
        }
        else
        {
            tagWithNs = badNs + ":" + elementName;
        }
        return tagWithNs;
    }
    ...
    {
        const ImageOperationXMLOptions outputOptions(false, false, includeIdentifiers);
        this->WriteXML(fname, volume, outputOptions);
    }

    void
    ImageStackDICOM::WriteXML(const std::string &fname, const UniformVolume &volume, const ImageOperationXMLOptions &outputOptions) const
    {
        ...
        if (modality == "mr")
        {
            ...
            {
                Coverity::FakeFree(mxmlNewReal(mxmlNewElement(x_modality, mungeXMLNs(std::string("dicom:GE"), std::string("GE"), "EffectiveEchoSpacing", outputOptions).c_str()), atof(firstImage->GetTagValue(DCM_GE_EffectiveEchoSpacing).c_str())));
            }

            if (firstImage->GetTagValue(DCM_SequenceName) != "" && outputOptions.m_includeIdentifiers)
            {
            ...
            if (firstImage->GetTagValue(DCM_GE_PulseSequenceName) != "" && outputOptions.m_includeIdentifiers)
            {
                Coverity::FakeFree(mxmlNewText(mxmlNewElement(x_modality, mungeXMLNs(std::string("dicom:GE"), std::string("GE"), "PulseSequenceName", outputOptions).c_str()), 0, firstImage->GetTagValue(DCM_GE_PulseSequenceName).c_str()));
            }

            if (firstImage->GetTagValue(DCM_GE_PulseSequenceDate) != "" && outputOptions.m_includeIdentifiers)
            {
                Coverity::FakeFree(mxmlNewText(mxmlNewElement(x_modality, mungeXMLNs(std::string("dicom:GE"), std::string("GE"), "PulseSequenceDate", outputOptions).c_str()), 0, firstImage->GetTagValue(DCM_GE_PulseSequenceDate).c_str()));
            }

            if (firstImage->GetTagValue(DCM_GE_InternalPulseSequenceName) != "" && outputOptions.m_includeIdentifiers)
            {
                Coverity::FakeFree(mxmlNewText(mxmlNewElement(x_modality, mungeXMLNs(std::string("dicom:GE"), std::string("GE"), "InternalPulseSequenceName", outputOptions).c_str()), 0, firstImage->GetTagValue(DCM_GE_InternalPulseSequenceName).c_str()));
            }
        ...
        if (outputOptions.m_includeIdentifiers)
        {
        ...
        if (file)
        {
            mxmlSaveFile(x_decl, file, Self::WhitespaceWriteMiniXML);
            fputs("\n", file); // end last line
            fclose(file);
        }
        else
